...
PARAM  <- list()
# PARAM$experimento <- "FE7310" #CAMBIO MR
PARAM$experimento <- "FE7310_MF05" #CAMBIO MR
# PARAM$exp_input  <- "DR7210" #CAMBIO MR
PARAM$exp_input  <- "DR7210_MF01" #CAMBIO MR
...
setorder( dataset, numero_de_cliente, foto_mes ) #CAMBIO MR

#agrego variables de aceleracion #CAMBIO MR
variables <- setdiff(names(dataset), c("numero_de_cliente", "foto_mes", "clase_ternaria")) #CAMBIO MR
for (var in variables) { #CAMBIO MR
  dataset[, paste0("acel_", var) := as.numeric(get(var)) - 2 * shift(as.numeric(get(var)), 1) + shift(as.numeric(get(var)), 2), #CAMBIO MR
          by = numero_de_cliente #CAMBIO MR
  ][, paste0("acel_", var) := ifelse(seq_len(.N) > 2, get(paste0("acel_", var)), NA)] #CAMBIO MR
} #CAMBIO MR
