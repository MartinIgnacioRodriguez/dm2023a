# EXP_03
# Se cambió el parámetro "weight" de las variables "dtrain" y "dvalidate"
...
PARAM  <- list()
PARAM$experimento  <- "HT6510_EXP04"

PARAM$exp_input  <- "TS6410_EXP04"
...
dtrain  <- lgb.Dataset( data=    data.matrix( dataset[ fold_train==1, campos_buenos, with=FALSE] ),
                        ...
                        weight=  dataset[ fold_train==1, weight ],
                        free_raw_data= FALSE
                      )
...
if( dataset[ fold_train==0 & fold_test==0 & fold_validate==1, .N ] > 0 )
{
  ...
  dvalidate  <- lgb.Dataset( data=  data.matrix( dataset[ fold_validate==1, campos_buenos, with=FALSE] ),
                             ...
                             weight= dataset[ fold_validate==1, weight],
                             free_raw_data= FALSE  )
