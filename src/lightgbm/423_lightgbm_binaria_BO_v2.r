# Aclaraciones MR
# Cambié: num_leaves y min_gain_to_split
...
PARAM$hyperparametertuning$semilla_azar  <- 100057  #Aqui poner su segunda semilla

#Aqui se cargan los bordes de los hiperparametros
#Search space
PARAM$hyperparametertuning$hs  <- makeParamSet(
         # As a general rule, if you reduce num_iterations, you should increase learning_rate.
         makeNumericParam("learning_rate",    lower=    0.01, upper=    0.3),
         
         # By default, LightGBM considers all features in a Dataset during the training process.  
         # Setting feature_fraction to 0.5, for example, tells LightGBM to randomly select 50% of 
         # features at the beginning of constructing each tree.
         ...
         # The "L" suffix ensures that the number is treated as an integer literal, rather than a floating-point number.
         makeIntegerParam("min_data_in_leaf", lower=    100L  , upper=  8000L),
         
         # A leaf-wise tree is typically much deeper than a depth-wise tree for a fixed number of leaves. 
         ...
         makeIntegerParam("num_leaves",       lower=   16L  , upper=  500L),
         makeIntegerParam("envios",           lower= 5000L  , upper= 15000L),
         
         # Params agregados por MR
         
         # Limit the tree depth explicitly.
         # Just choose a value between 3 and 12 (this range tends to work well on Kaggle for any dataset).
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  param_basicos  <- list( 
                          # Objective specifies the type of learning task like binary, multiclass
                          ...
                          # Each tree in the ensemble builds on the predictions of the last tree
                          # — i.e., each boosting round is an improvement of the last.
                          # For example, setting early_stopping_rounds to 100 means we stop the training if the predictions 
                          # have not improved for the last 100 rounds.
